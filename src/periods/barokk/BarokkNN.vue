<template lang="pug">
PagePeriod(
  :bannerImage="require(`./barokk.jpg`)"
  :circleImage="require(`./sirkel_barokk.png`)"
  )
  // Content here
  h3 Tida og samfunnstilhøva 

  h5 Kyrkja mister makt 
  figure.col.col6.right-image.mt5
    img(src="./Barokk_Tida_og_samfunnsforholdene1.jpg" alt="Portrettbilete av tre vaksne, ei lita jente og ein leikande hund. Måleri.")
    figcaption Ludvig XIV av Frankrike og familien hans

  p.mt0
    | Kyrkja mister mykje av den makta ho hadde i mellomalderen, men denne makta går ikkje til folket. Det er kongane i dei ulike landa i Europa som samlar makta og får større kontroll over kyrkja. 

  h5 Konge av Guds nåde 
  p
    | Dei som har makt i samfunnet, blir dyrka. Det er stor avstand mellom dei få rike og resten av folket. Herskarane utnyttar religionen til å rettferdiggjere makt og posisjon. Ifølgje Luther har kongen fått makta si frå Gud fordi all makt kjem frå Gud. 

  .clear

  h5 Eineveldet 

  figure.col.col6.right-image.mt5
    img(src="./Barokk_Tida_og_samfunnsforholdene2.jpg" alt="Mann i profil. Portrettmåleri.")
    figcaption Kong Fredrik 3. av Danmark og Noreg

  p.mt0
    | I 1660 blir Fredrik 3. eineveldig konge over Danmark og Noreg. Det vil seie at han får uavgrensa makt. Kongen konfiskerer delar av dei kyrkjelege eigedommane etter reformasjonen og skaffar seg dermed meir rikdom. 

  h5 Merkantilisme 
  p
    | Omgrepet merkantilisme stammar frå opplysningstida. Dei store sjøfartsnasjonane driv stor handelsverksemd, og ideen er å eksportere fleire varer enn dei importerer. På den måten legg ein til rette for større statsmakt, der kongen styrer all handel og skjermar produksjonen. Dette fører i mange tilfelle til konflikt mellom ulike statar, og mellom kongen og det nye handelsborgarskapet  

  h3 Litt om tenkjemåten 

  h5 Eineveldet 
  figure.col.col4.right-image.mt0
    img(src="./Barokk_Litt_om_tenkemaaten.jpg" alt="Bord med hovudskalle. Illustrasjon.")
    figcaption Detalj frå tittelstikket på førsteutgåva av boka Taare-Offer av Dorothe Engelbretsdotter  

  p.mt0 
    | Samfunnsbiletet (verdsbiletet) kan samanliknast med ein pyramide der alle menneska har sin faste plass. På toppen står kongen, og over kongen står berre Gud. Under kjem dei adelege, deretter borgarskapet med handelsmenn og handverkarar. Bønder, leiglendingar og husmenn står nedst i hierarkiet. 

  h5 Grip dagen, men hugs at du skal døy 
  p 
    | Livshaldninga til menneska på 1600-talet kan karakteriserast med dei to latinske uttrykka Carpe diem, som tyder ”Grip dagen”, og Memento mori, som tyder ”Hugs at du skal døy”. Livet er kort, alt er forgjengeleg, så grip dagen og lev mens du kan. Døden ventar, og den kjem vi ikkje utanom. Desse to uttrykka seier noko om den motsetningsfylte livshaldninga som ofte syner seg i barokk kunst. 

  h5 Idealisme 
  p 
    | Ein del tenkjarar på 1600-talet meiner at tilværet er av sjeleleg eller åndeleg karakter. Ei slik haldning kallar vi idealisme. I motsetning til materialismen ser idealismen verda som noko som går ut frå medvitet. Ein idealist i moderne språkbruk er ein som handlar ut frå visse ideal. 

  h5 Materialisme 
  p 
    | Andre meiner at alt kan førast attende til fysiske storleikar. Ei slik haldning kallar vi materialisme. Den engelske legen William Harvey oppdaga i 1628 at hjartet berre var ein muskel som pumpa blodet rundt i menneskekroppen. Ein kollega av Harvey uttrykte: Video sed non credo – ”Eg ser det, men eg trur det ikkje”. Denne konflikten mellom gamle, fastgrodde førestillingar og vitskaplege prov pregar barokken. Materialismen avviser med andre ord at det eksisterer noko som er overnaturleg, og hevdar at alt lét seg forklare vitskapleg. 

  h3 Kjennemerke ved barokkstilen 

  figure.col.col6.left-image.mt10
    img(src="./Barokk_Kjennetegn_ved_barokkstilen1.jpg" alt="Stort pipeorgel som strekk seg opp mot taket. Foto.")

  h5 Barokken som stilart 
  p 
    | Barokken er nemninga på ein stilart som oppstår i Roma på slutten av 1500-talet. I tillegg er barokken namnet på ein kulturhistorisk epoke som fyller heile 1600-talet. Barokken omfattar europeisk kunst, arkitektur, musikk og litteratur. I Noreg ser vi påverknaden noko seinare enn i resten av Europa, og her i landet reknar vi derfor den barokke perioden frå ca. 1650 til 1750. 

  h5 Den katolske kyrkja som pådrivar 
  p 
    | Kyrkja bruker den barokke stilarten til å manifestere makta si. Vi kan derfor sjå barokken som ein motreaksjon til renessansen. Det aller meste av barokk kunst er kyrkjekunst, og det meste av litteraturen er salmar og dikting knytt til kyrkja. 

  h5 Ein prangande stil 
  p 
    | I barokken møter vi ei pompøs og prangande livsutfolding, med vekt på det overdådige. Skulptur- og målarkunst, barokk arkitektur og møbelstil er prega av ei slik overveldande utsmykking. Det blir ikkje spart på verken materiale eller effektar.  

  figure.col.col4.left-image.mt5
    img(src="./Barokk_Kjennetegn_ved_barokkstilen2.jpg" alt="Mann blir korsfesta og tre personar prøver å løfte korset. Måleri.")    
    figcaption
      em Peters korsfesting
      | . Måleri av Caravaggio

  h5 Konflikt og dramatikk 
  p 
    | Heller ikkje forfattarane er smålåtne i bruken av verkemiddel. Dei bruker svulstige uttrykk og sterke metaforar. Overdriving, motsetningar og eit sterkt og dramatisk biletspråk er kjennemerke på barokk kunst av alle slag. Operaen som scenekunst voks fram i den barokke perioden, og store, påkosta operaframsyningar kom på mote.  

  h5 Det åndelege og det verdslege  
  p 
    | Gud står i sentrum i barokken, men kunsten er likevel dualistisk. Det vil seie at kunstnaren fokuserer på både mennesket (det verdslege) og Gud (det åndelege). Kunstnaren fokuserer altså både på dette livet og på livet etter døden. 

  figure.col.col4.right-image.mt5
    img(src="./Barokk_Kjennetegn_ved_barokkstilen3.jpg" alt="Portrett av person med fiolin.  Måleri.")
    figcaption Antonio Vivaldi

  figure.col.col4.right-image.mt5
    img(src="./Barokk_Kjennetegn_ved_barokkstilen4.jpg" alt="Mann med et ark i handa. Måleri.")
    figcaption Johann Sebastian Bach

  h5 Sterke kontrastar 
  p 
    | Det er dei sterke kontrastane som pregar barokken. Desse kontrastane kjem òg til uttrykk gjennom musikken. Barokk musikk er lett å dra kjensel på med sine sterke kontrastar, slik vi kjenner det gjennom 
    em Dei fire årstidene 
    | av den italienske komponisten Vivaldi. Andre kjende komponistar frå perioden er Bach og Händel (Tyskland). Begge er særleg kjende for å komponere kyrkjemusikk.


  h3 Litteraturen 

  figure.col.col6.left-image.mt5
    img(src="./Barokk_Litteraturen.jpg" alt="Tenkande kvinne som skriv i ei notatbok. Illustrasjon.")
    figcaption Tittelstikket på førsteutgåva av boka Taare-Offer av Dorothe Engelbretsdotter

  h5 Religiøs litteratur 
  p
    | Den barokke litteraturen er først og fremst religiøs, og det er salmediktinga som dominerer. Gjennomgangsmotivet i dei barokke salmane er dødstanken: alt skal gå under – det verkelege livet er i himmelen. Slik sett er det ofte ein lengt etter døden som kjem til uttrykk. 

  h5 Europeiske forfattarar 
  p
    | I Italia er Giambattista Marino den fremste barokkdiktaren. I Spania er det Francisco de Quevedo, og i England er det John Donne og John Milton. I Tyskland skriv Martin Luther barokke salmar. Han inspirerer nok Thomas Kingo, som skulle bli den største nordiske salmediktaren i denne perioden. 
  
  div.flex.flex-wrap
    div.col.col4
      figure.figure-left.mt10
        img(src="./Barokk_Thomas_Kingo.jpg" alt="Prest i sort kappe og kvit krage som les i ei bok. Illustrasjon.")
    div.col.col8
      h5 Thomas Kingo (1634–1703) 
      p
        | er dansk diktar og biskop. I 1699 kjem det ut ei salmebok som inneheld 86 av salmane hans. Denne boka – 
        em Kingos salmebok 
        | – blir godkjend av kongen til bruk i alle kyrkjer i Danmark og Noreg, og er i bruk heilt fram til 1870-åra. Boka finn du på Tidslinja. 

  div.flex.flex-wrap
    div.col.col4
      figure.figure-left.mt10
        img(src="./Barokk_Engelbretsdotter.jpg" alt="Portrett av kvinne med elegant hatt. Illustrasjon.")

    div.col.col8
      h5 Dorothe Engelbretsdotter (1634–1716) 
      p
        | er fødd og lever i Bergen. Ho er populær i samtida si og kan leve av boksalet. Det er ganske utruleg at ei kvinne kan klare det på 1600-talet. Men i privatlivet opplever ho mykje motgang og sorg. Boka 
        em Siælens Sang-Offer 
        | finn du på Tidslinja. 

  div.flex.flex-wrap
    div.col.col4
      figure.figure-left.mt10
        img(src="./Barokk_Dass.jpg" alt="Prest i sort kappe og kvit prestekrage som held ei bok. Måleri.")
    div.col.col8
      h5 Petter Dass (1647–1707) 
      p
        | kjem frå Nordland og er ein populær mann i si samtid. Han blir rekna for vår største barokkdiktar, sjølv om stilen hans er mindre tidstypisk enn stilen til Kingo og Engelbretsdotter. Boka 
        em Nordlands Trompet 
        | finn du på Tidslinja. 

  h3 Litt om språket  

  h5 Talemålet: frå norrønt til moderne norsk 
  p
    | Det norrøne talemålet er borte, og det moderne norske talespråket (dialektane) har festa seg, ikkje ulikt den forma dei har i dag. 

  h5 Skiljet mellom vestnorsk og austnorsk 
  p
    | Det norrøne skriftspråket er utdøydd, men talespråket har overlevd og utvikla seg vidare. Skiljet mellom aust og vest festar seg på 1500-talet. 

  h5 Bergen og nedertysk 
  p
    | Bergen og Vestlandet har hatt ein sterk språkleg påverknad frå den hanseatiske (tysk-nederlandske) handelsverksemda som har gått føre seg heilt sidan 1200-talet. Denne påverknaden held fram og fargar både talemålet og skriftspråket i Bergen. 


  h5 Bibelen på dansk 
  figure.col.col6.right-image.mt5
    img(src="./Bibel-Chr._III_-_1.jpg" alt="Forsida til eldre utgåve av Bibelen. Illustrasjon.")
    figcaption Tittelbladet på bibelen til Kristian 3. frå 1550
      br
      a(href="media/periods/barokk/Bibel1550.jpg" target="_blank") Sjå ark fra bibelen
      NdlaAudio(
        source="media/periods/barokk/tekst2_Christian_III_bibel_juleevangeliet.mp3"
      )
  p
    | Det kyrkjelege språket er dansk fordi det blei brukt av danske prestar. Både bibel, katekisme og salmebok er prenta på dansk. Bibelen kjem ut på dansk i 1550, og det er denne omsetjinga som blir brukt her i landet heilt fram til 1904. 

  h5 Administrasjonsspråket 
  p
    | Dansk er administrasjonsspråket på 1600-talet. Dei fleste embetsmenn er danske, og dei skriv og taler dansk. Offentlege dokument og sakspapir (såkalla diplom) blir skrivne på dansk. 

  h5 Bygdemålstekstar  
  p
    | Det finst likevel tekstar med innslag av dialekt og norsk syntaks. Dette er tekstar som blir nytta til bryllaup og festlege lag. Det finst også jordbøker (dokument om eigedommar), lover og brev der teksten er farga av dialekten til forfattaren. Slike tekstar kallar vi gjerne bygdemålstekstar. 

  p
    a(href="media/periods/barokk/anfind.html" target="_blank") Les folkevisa ”Anfind han gjenghir” frå ”Tiende lystige Norske Viser”


  h5 Norvagismar 
  p
    | Både Petter Dass og Dorothe Engelbretsdotter skriv på dansk, men dei bruker norske ord og uttrykk somme stader. Slike ord kallar vi norvagismar. Eit eksempel kan vere 
    em foss 
    | for det danske ordet ”vandfald”. Petter Dass brukte elles norske ord når han omtalte fiskeslag og fangstreiskap.  

  h3 Biletkunst og arkitektur

  figure
    img(src="./Barokk_Bildekunst_og_arkitektur1.jpg" alt="Innsida av ei kjerke med utsmykkingar i marmorgolvet og i taket. Foto." style="min-height: 305px; object-fit: cover;")
    figcaption Berninis baldakin og kor i Peterskyrkja

  div.col.col6.display-ib.p5
    figure
      img(src="./Barokk_Peter-Paul-Rubens---nederst.jpg" alt="To nakne kvinner som er omfavna av to menn, samt to hestar og eit barn.  Måleri.")
      figcaption Peter Paul Rubens: 
        em Døtrene til Leukippos blir røva

  div.col.col6.display-ib.p5
    figure
      img(src="./Barokk_Bildekunst_og_arkitektur3.jpg" alt="Skulptur av bøygd hovud med krøllete hår. Foto." style="min-height: 305px; object-fit: cover;")
      figcaption Detalj frå skulpturen 
        em Den heilage Teresas ekstase 
        |  av Giovanni Lorenzo Bernini

</template>

<script>
import PagePeriod from "@/components/PagePeriod";
import NdlaVideo from "@/components/NdlaVideo";
import NdlaAudio from "@/components/NdlaAudio";

export default {
  name: "Book",
  components: {
    NdlaAudio,
    NdlaVideo,
    PagePeriod,
  },
};
</script>
